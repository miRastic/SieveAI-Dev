from ..process.rescoring import PluginRescoringBase

class AnnapuRNA(PluginRescoringBase):
  is_ready = False
  plugin_name = "AnnapuRNA"
  plugin_uid = "AnnapuRNA"
  plugin_version = "1.0"
  assignments = ['rescoring']
  ...
  def __init__(self, *args, **kwargs):
    ...
    # Set all defaults
    [setattr(self, _k, __defaults[_k]) for _k in __defaults.keys() if not hasattr(self, _k)]
    __defaults = dict() # Unset defaults to prevent running for second time
  ...
  def __read_results_df(self, *args, **kwargs):
    ...
    if not self.check_path(_score_file_path):
  ...
  def annapurna_merge_rescoring(self, *args, **kwargs):
    ...
    _score_file_path = f"{self.path_base}{OS.sep}{self.file__result_score}"
    self.log_info(f"Processing {_results.shape}.")

    for _complex_id in _results['complex_id'].unique():
      ...
      for _am in self.annapurna_models:
        ...
        if self.check_path(_m_res_f):
          _model_res_df = PD.read_csv(_m_res_f, sep="\t")
          _model_res_df.columns = _updated_cols
          _model_res_df['complex_id'] = _model_res_df.apply(lambda row: self.file_name(row['complex_id'], num_ext = 2), axis=1)
          ...
          self.log_error(f"{_m_res_f} file doesn't exist.")
      ...
      self.log_info(f"Merge processed {_complex_id}.")
    ...
    self.log_info(f"Processed and Merged {_results.shape} to {_score_file_path}. Took {self.time_elapsed()}")
  ...
  def rescoring_AnnapuRNA(self, *args, **kwargs):
    ...
    if not isinstance(_results, PD.DataFrame):
      self.log_info(f"{_results}. Skipping rescoring_AnnapuRNA...")
      return

    _models = " ".join(["-m " + _m for _m in self.annapurna_models])
    self.dir_rescoring = self.validate_dir(f"{self.path_base}/rescoring-dl-modern-2")
    self.path_receptor_pdbqt = f"{self.path_base}/{self.dir_receptor_pdbqt}"
    self.path_docking = f"{self.path_base}/{self.dir_docking}"

    for _idx, _complex in _results.iterrows():
      ...
      if self.check_path(_complex_output_dir):
        self.log_info(f"{_complex_uid} already analysed.")
        continue
      ...
      # print(_command)
      _result = self.cmd_run(_command)
      self.log_info(f"Rescoring processed {_complex_uid}.")

    self.log_info(f"Total Time Taken %s." % self.time_elapsed())
